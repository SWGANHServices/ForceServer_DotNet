    ...
    public static class PacketUtilities
    {
        ...
        public static byte[] EncryptMessage(byte[] data, int length, int crcSeed)
        {
            ...
            using (var binaryWriter = new BinaryWriter(writtenMemory))
            {
                ...
                for (var i = data.Length - 2; i < data.Length; i++) binaryWriter.Write(data[i]);
        ...
        public static IEnumerable<byte> AppendCrc(byte[] encoded, int length, int crcSeed)
        {
            using (var writtenMemory = new MemoryStream())
            using (var binaryWriter = new BinaryWriter(writtenMemory))
            {
                const short crcLength = 2;
                var crc = GenerateCrc(encoded,
                    (short) (length - crcLength), crcSeed);
                var newOutput = encoded;
                for (short i = 0; i < crcLength; i++) newOutput[length - 1 - i] = (byte) ((crc >> (8 * i)) & 0xFF);
                binaryWriter.Write(newOutput);
                return writtenMemory.ToArray();
            }
        }

        private static int GenerateCrc(IReadOnlyList<byte> encoded, short length, int crcSeed)
        {
            var arrayIndex = ~crcSeed & 0xFF;
            var nCrc = CrcTable[arrayIndex];
            nCrc ^= 0x00FFFFFF;
